...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all=False):
        ...
        for idx, blk in enumerate(self.transformer.resblocks[:-1], start=1):
            # x = blk(x)
            x_out, attn_out = self.vanilla_attn(blk.attn, blk.ln_1(x))  # attn_out: 12, 197, 197
            x = x + x_out
            x = x + blk.mlp(blk.ln_2(x))
            x_attn_out_list.append(x+attn_out)
        ...
        feats = x_attn_out_list[self.pre_adjust_idx][1:, ...].clone()
    ...
    def custom_attn(self, attn_layer, x, mid_simi):
        ...
        v = v.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)  # 12,197, 64
        ...
        agg_conv = nn.Conv2d(768, 768, kernel_size=3, padding=1, groups=768, bias=False).to(device).half()
        with torch.no_grad():
            agg_conv.weight.fill_(1.0)

        # computing the information entropy scores
        prob = attn_weights.view(num_heads, -1)
        # probabilities = attn_MHSA_flat / attn_MHSA_flat.sum(dim=-1, keepdim=True)
        # 计算信息熵，为避免 log2(0) 产生 NaN，添加一个极小值
        EPS = 1e-6
        metrics = -torch.sum(prob * torch.log2(prob + EPS), dim=-1)

        sorted_metrics, sorted_indices = torch.sort(metrics)
        sorted_metrics = sorted_metrics[:self.topk_SHA]
        #        print(metrics)

        # the diversity of multi-head self-attention
        # diversity of multi-head self-attention
        v_orig = v.transpose(0, 1)  # 197, 12, 64
        v_orig = v_orig.contiguous().view(-1, l, embed_dim)  # 1, 196, 768

        attn_MHSA = attn_weights  # 12, 196, 196
        sim_MHSA = mid_simi + k_attn

        v_out = torch.zeros_like(v_orig)  # 196, 1, 768
        for i in range(num_heads):
            if metrics[i] in sorted_metrics:
                attn_MHSA_head = attn_MHSA[i, :, :].unsqueeze(0)
                sim_MHSA_head = sim_MHSA[i, :, :].unsqueeze(0)  # 1, 196, 196
                v_orig_head = torch.bmm(attn_MHSA_head, v_orig)  # 1, 196, 768
                v_out += v_orig_head  # 1, 196, 768

        v_out = v_out / num_heads
        attn_output = v_out.transpose(0, 1).contiguous()
        attn_output = attn_layer.out_proj(attn_output)  # 196, 1, 768

        return attn_output

    def vanilla_attn(self, attn_layer, x):
        num_heads = attn_layer.num_heads
        ...
        # the diversity of multi-head self-attention
        v_orig = v.transpose(0, 1)  # 197, 12, 64
        v_orig = v_orig.contiguous().view(-1, l, embed_dim)  # 1, 197, 768

        attn_MHSA = torch.bmm(k * scale, k.transpose(1, 2))  # 12, 197, 197
        attn_MHSA = F.softmax(attn_MHSA, dim=-1)

        v_out = torch.zeros_like(v.transpose(0, 1).contiguous().view(bsz, l, embed_dim))  # 1, 197, 768
        for i in range(num_heads):
            if metrics[i] in sorted_metrics:
                attn_MHSA_head = attn_MHSA[i, :, :].unsqueeze(0)  # 1, 197, 197
                v_orig_head = torch.bmm(attn_MHSA_head, v_orig)  # 1, 197, 768
                v_out += v_orig_head  # 1, 197, 768

        v_out = v_out
        v_out = v_out.transpose(0, 1).contiguous()
        v_out = attn_layer.out_proj(v_out)

        attn_output = torch.bmm(attn_weights, v)
        attn_output = attn_output.transpose(0, 1).contiguous().view(-1, bsz, embed_dim)
        attn_output = attn_layer.out_proj(attn_output)

        return attn_output, v_out
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        image_features, out_ies = self.encode_image(image)
        ...
        return logits_per_image, logits_per_text, out_ies
